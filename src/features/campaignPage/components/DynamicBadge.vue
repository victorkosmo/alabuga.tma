<template>
  <div class="flex flex-col items-center justify-center p-2 border rounded-md text-center text-xs font-medium leading-tight w-full h-full">
    <!-- Achievement Lock -->
    <div v-if="type === 'achievement-lock' && achievement" class="relative w-full h-full flex items-center justify-center">
      ...
      <div class="absolute bottom-1 right-1 bg-background rounded-full p-0.5">
        <Lock class="h-5 w-5 text-destructive" />
      </div>
    </div>
    ...
    <!-- Pending Review -->
    <div v-else-if="type === 'pending-review'" class="flex flex-col items-center justify-center gap-2 text-muted-foreground">
      <Clock class="h-8 w-8" />
      <span>На проверке</span>
    </div>

    <!-- Scan QR -->
    <div v-else-if="type === 'scan-qr'" class="flex flex-col items-center justify-center gap-2 text-muted-foreground">
